...
| **M2** | Permission-driven, department-aware approval authority | 🔴 The user's actual request. Migrate `RequisitionsController` + `ApprovalChainsController` off `RequireRole` literals onto `[HasPermission(...)]` so the seeded codes stop being decorative. **Touches authorization — `security-reviewer` required before this is called done (CLAUDE.md).** Needs an ADR. | M1 | NOT STARTED |
| **M2.5** | **Revise-and-resubmit a rejected requisition** | 🔴 User-requested 2026-08-14. Today `Rejected` is terminal and `UpdateAsync` permits edits only in `Draft`, so a rejection can never be fixed — a new requisition must be raised, losing the thread. **Changes a documented rule**, so it needs an ADR + module-doc update *before* code, and a schema change for round history (see contract below). Shares `RequisitionService`/`RequisitionsController` with M2, hence the ordering. | M2 | NOT STARTED |
