...
| ADR-0025 step 3 | 🚧 **3a + 3b done 2026-08-21, 3c in progress** | The preset **is** V1.0 now — a copy of `design/internal/ds.js` plus a fenced compat block aliasing the old names onto it, so both apps already render V1.0 colours and only the class *names* are old. Base CSS and fonts ported from `ds.css`. Remaining: **732 compat usages** in source (`features` 324, `pages` 189, `packages/ui` 116, `components` 86, `public/app` 17). Delete the compat block when that count is zero — the command that reports it is in NEXT-SESSION §0 |
