...
| **The Interviews screen's "Upcoming" filter is not upcoming** | 🟠 High | Found by using the screen 2026-08-29: **"Upcoming" and "All" return identical rows.** The tab sends no `status`, and `InterviewService` reads that as *every status except Cancelled* — so a recruiter opening "Upcoming" to see what is next is shown rounds that finished last week (here: two `Completed` rounds dated Aug 17 and Aug 20, on Aug 29). The behaviour is deliberate and even documented in `InterviewsPage.tsx` and pinned by a test; **the label is what nobody checked against it.** The kit draws the tab as "Upcoming" too, so the design carries the same assumption. Not fixed, because the right answer is a product call: filter to `Scheduled`/`NoShow` (rounds still needing something — note a *past* Scheduled round is work to chase, so a pure date filter would hide it), or rename the tab to "Active". **Ask before changing** |
