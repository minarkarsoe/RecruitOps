...
| ADR-0025 steps 3–4 not started | 🔴 **High — raised 2026-08-21** | The 25 design screens are on V1.0 tokens; `packages/ui/tailwind-preset.js` and both frontends are still on Clear Pipeline. **Two token systems running in parallel is the exact condition ADR-0025 was written to end** — reproduced, just in the other direction. Raised from Medium because `design/` is now the declared source of truth for UI (CLAUDE.md), and the two do not share *class names* — a screen copied from the kit renders unstyled, not off-brand. ~1,120 old-token usages to move; **zero** design-kit tokens present today. See NEXT-SESSION §0 for the measured breakdown |
| **163 shipped classes emit no CSS** | 🔴 High | Verified 2026-08-21 by building `frontend/internal` and grepping `dist/assets/*.css`: `text-ink-500` (57 uses), `text-ink-700` (39), `text-ink-800` (20), `border-line-300` (28), `bg-surface-100` (13), `border-line-100` (5), `bg-surface-200` (1) are **ABSENT** from the built stylesheet. The preset defines `ink` at 900/600/400, `surface` at 0/50, `line` at 200 — the other steps were never defined, so those elements silently inherit their parent's colour. Not a migration cost: a live defect the ADR-0025 step 3 migration happens to fix |
