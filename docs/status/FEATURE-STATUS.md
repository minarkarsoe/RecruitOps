...
> ✅ **Backend: 533/533 green** (57 domain + 476 api), re-run 2026-08-20 after ADR-0026 step 1. Covers Module 1 end to end, login
...
| ⚠️ **Migration `AddOutboundDeliveryAndScheduledJobs` is generated but NOT applied** | 🟠 Action needed | `20260820072400`. Creates `OutboundMessages` and `ScheduledJobs` with three indexes and three check constraints. Per CLAUDE.md it is proposed, not applied — **a human runs `dotnet ef database update` on a dev database**. Until then the entities exist in code and the tables do not, so anything reading them at runtime fails |
...
| **No email sender anywhere in the codebase** | 🟠 High · **decided, not yet built** | `grep` for `SmtpClient\|IEmailSender\|MailKit\|SendGrid` across `backend/src` returns nothing. Blocks Module 3 interview invitations, Module 4 offer sends + reminders + the IT/Admin handoff, Module 5 scheduled reports and Module 8 notifications. **One capability, five modules.** [ADR-0026](../decisions/ADR-0026-outbound-delivery-and-background-jobs.md) is **Accepted 2026-08-18**: SMTP behind `IEmailSender`, a transactional `OutboundMessage` outbox, one in-process `BackgroundService`, **no new package**. Unblocked — it is now the top backlog item |
| **A background job has no tenant** | 🟠 High · new, arrives with ADR-0026 | `CurrentTenant` reads `IHttpContextAccessor`, so in a job `TenantId` is `Guid.Empty`: every global query filter matches nothing, and `AppDbContext` would stamp inserts with tenant `Guid.Empty`. ADR-0026 §4 decides the worker sets a scoped tenant from the message row rather than extending the `IgnoreQueryFilters()` workaround into handlers. **This makes `ICurrentTenant` settable — a security-critical seam — and needs its own review plus a two-tenant isolation test** |
