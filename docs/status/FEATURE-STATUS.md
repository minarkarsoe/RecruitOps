...
| Step 3 has **not** been security-reviewed | 🟠 High | The 2026-08-20 review covered `a2de09c` (the tenant seam). The invitation handler landed **after** it and is the first code to read candidate data with no user behind it: it queries applications and candidates under the tenant filter alone, with **no department scoping** — deliberate per ADR-0026 §4 ("a job is not a user"), and therefore exactly the kind of deliberate hole worth a second pair of eyes. It also puts a candidate's name and interview details on the wire. CLAUDE.md requires a `security-reviewer` pass here |
