  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold tracking-tight">Role Builder & Permissions</h1>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {roles.map((roleItem) => (
            <div
              key={roleItem.id}
              className="border border-line rounded-lg p-5 bg-white shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between"
            >
              <div>
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <div className="flex flex-col items-end gap-1">
                    {roleItem.isSystemRole ? (
                      <span className="inline-block px-2 py-0.5 rounded text-2xs font-semibold bg-line text-ink-700">
                        System Role
                      </span>
                    ) : (
                      <span className="inline-block px-2 py-0.5 rounded text-2xs font-semibold bg-brand-50 text-brand-700">
                        Custom Role
                      </span>
                    )}
                    {roleItem.isSuperAdmin && (
                      <span className="inline-block px-2 py-0.5 rounded text-2xs font-bold bg-amber-100 text-amber-900">
                        Super-Admin
                      </span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-4 text-xs text-ink-500 border-t border-line pt-3 mb-4">
                  <div>
                  ...
                  <>
                    ...
                    {hasPermission(currentSession, 'permission:roles:roles:delete') && (
                      <button
                        ...
                        className="text-xs font-semibold text-critical-700 hover:text-critical-700 disabled:opacity-40 disabled:cursor-not-allowed px-2 py-1 rounded hover:bg-critical-50"
                      >
                        Delete
                      </button>
                    )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 overflow-y-auto">
          <div className="relative w-full max-w-4xl bg-white rounded-xl shadow-2xl overflow-hidden my-8 max-h-[90vh] flex flex-col">
            <div className="bg-canvas px-6 py-4 border-b border-line flex items-center justify-between shrink-0">
              <div>
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-ink-700 mb-1">
                    Role Name <span className="text-critical-700">*</span>
                  </label>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full text-sm px-3 py-2 border border-line-strong rounded-md focus:ring-1 focus:ring-brand-50 disabled:bg-canvas"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full text-sm font-mono px-3 py-2 border border-line-strong rounded-md bg-canvas text-ink-700 disabled:opacity-75"
                  />
                </div>
              </div>
              ...
              <div className="border-t border-line pt-4 flex items-center justify-end gap-3 shrink-0">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                ...
                {modalMode !== 'view' && !isSystemRoleModal && (
                  <button
                    type="submit"
                    disabled={saving}
                    className="px-4 py-2 text-sm font-semibold text-white bg-brand-700 hover:bg-brand-700 rounded-md shadow-sm disabled:opacity-50"
                  >
          ...
          <div className="bg-white rounded-lg max-w-md w-full p-6 shadow-xl space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button
                ...
                disabled={deleting}
                className="px-4 py-2 text-sm font-semibold text-white bg-critical-700 hover:bg-critical-700 rounded-md shadow-sm disabled:opacity-50"
              >
